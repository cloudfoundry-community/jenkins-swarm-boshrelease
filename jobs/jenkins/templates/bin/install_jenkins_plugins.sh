...
installPlugin() {
  if [ -f ${plugin_dir}/${1}.hpi -o -f ${plugin_dir}/${1}.jpi ]; then
    ...
    echo "Downloading Jenkins plugin: $1"
...
while [ "$changed"  == "1" ]; do
  ...
  for f in ${plugin_dir}/*.hpi ; do
    deps=$( unzip -p ${f} META-INF/MANIFEST.MF | tr -d '\r' | awk '/^Plugin-Dependencies:/{ ORS=""; print $0; while ((getline n) && n~"^ ") { sub(/^[ \t\r\n]+/, "", n); print n } }' | awk '{ print $2 }' | tr ',' '\n' | awk -F ':' '{ print $1 }' | tr '\n' ' ' )
